...
**Resolved 2026-07-06 via game data — the tech exists.** `EMBA0` in the ADMIN tech tree
(v71), in-game name **"Embassy"**: "Unlocks a place that lets us improve relations with
other factions and manipulate their courts" (`text/tech/ADMIN.txt`). 5 levels, costs Civic
Knowledge (5 base, +5 per level), no prerequisites; each level `ROOM_EMBASSY>ADD: 0.2` →
+20% embassy output per level (+100% at max). See [[2026-07-06-gamedata-embassy-tech]].
...
- Adjacent facts: gifting cloth/jewelry/sithilon gives embassy points *(community)*; v71 patch notes add an **Emissary office for nobles** and state emissary allocation "starts to matter a great deal" *(patch-note)*.
