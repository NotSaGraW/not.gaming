...
Principle (curator + cataloguer synthesis, sources below): **planning depth should match
information.** At t=0 you know the least the run will ever know; plan deeply only what is
frozen, sketch what is structural, leave the rest to the run. Overplanning is spending
hours optimizing gradients the game deliberately flattened.

**Worth choosing carefully — the two frozen variables:**
...
- **Region** — area × moisture caps maximum population ([[population-thresholds]]); climate is the one fulfillment the dev manual calls impossible to fix later (Ch12).
...
- Pasture land: per-worker yield is fertility-independent.
...
**Planning tool = skeleton, not city:** reserve wide main roads (decorations retrofit into
them, Ch12), species-district boundaries with entrance separation ([[brawls]]), and noise
zones ([[overlays]]). Buildings get discovered by the run.
